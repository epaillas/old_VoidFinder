...
program overlapping
  ...
  allocate(pos_arr(3, nv), rvoid_arr(nv), ng_arr(nv), nden_arr(nv), mark(nv))
  ...
  do i = 1, nv
    if(mark(i) .eq. 0) then
      count = count + 1
      write(11, '(4F10.3, 1I10, 1F10.3)') &
      & pos_arr(1,i), pos_arr(2,i), pos_arr(3,i), rvoid_arr(i), ng_arr(i), nden_arr(i)
    endif
  end do
