...
program recentring_2D
  ...
  integer :: i, j, ii, ix, iy
  integer :: ipx, ipy, ndif, stuck
  ...
  if (id == 0) write(*,*) '-----------------------'
  if (id == 0) write(*,*) 'Running recentring_2D.exe'
  ...
  do i = 1, nc
    ...
    if(mod(i, process_num) .eq. id) then
      ...
      do j = 1, nrc
        ...
        do ii = nrbin, 1, -1
          rnd_rvoid = rwidth * ii
          rnd_ng = crbin_data(ii)
          rnd_nden = crbin_data(ii) / crbin_rand(ii)
          if (rnd_nden .lt. delta .and. rnd_rvoid .gt. rvoid&
