program vg_ccf_r_mu
    ...
    real(dp) :: velx, vely, velz, voidcen_vel
    ...
    real(dp), dimension(3) :: voidcen, r, v
    ...
    if (iargc() .ne. 10) then
      ...
      write(*,*) '-----------------------'
      write(*,*) 'Running vg_ccf_voidcen_velocity.exe'
      ...
      write(*, *) 'rmin: ', trim(rmin_char), ' Mpc/h'
      write(*, *) 'rmax: ', trim(rmax_char), ' Mpc/h'
      write(*, *) 'nrbin: ', trim(nrbin_char)
      write(*, *) 'min_rv: ', trim(min_rv_char), ' Mpc/h'
      write(*, *) 'max_rv: ', trim(max_rv_char), ' Mpc/h'
