...
typedef struct Zone {
  ...
  float vol; /* Total volume of all particles in the zone */
  float voljoin; /* Total volume of all particles in the joined void */
} ZONE;
...
int main(int argc,char **argv) {
  ...
  float lowvol, voltol, borderdens, invborderdens, prob;
  ...
  float maxdens, maxdens_border, mindens, mindens_border;
  float *sorter, maxdenscontrast;
  int *iord;
  ...
  if (sscanf(argv[3],"%f",&voltol) == 0) {
    printf("Bad density threshold.\n");
    exit(0);
  }  
  if (sscanf(argv[4],"%f",&borderdens) == 0) {
    printf("Bad density threshold.\n");
    exit(0);
  }
  if (voltol <= 0.) {
    ...
    sprintf(zonefile,"%s.%f.zone",prefix,voltol);
    sprintf(voidfile,"%s.%f.void",prefix,voltol);
    sprintf(txtfile,"%s.%f.txt",prefix,voltol);
  }
  if (borderdens == 0.) {
    borderdens = 1.1e-30;
  }
  ...
  sorter = (float *)malloc((nzones+1)*sizeof(float));
  ...
  for (h = 0; h<nzones; h++) {
	...
	fprintf(vod,"%d %f ",nhl2, lowvol/p[z[h].core].dens);
	for (h2 = 0; h2 < nhl2; h2++) {
    ...
    if (z[h].denscontrast > maxdenscontrast) {
      maxdenscontrast = (float)z[h].denscontrast;
    }
  ...
  for (h=0; h< nzones; h++) {
	...
	((obo == 'v')&&(p[z[h].core].dens < invborderdens))) {
      sorter[h] = (float)z[h].denscontrast;
      nvoidsreal ++;
    } else {
      sorter[h] = 0.;
    }
